...
public class SqsExplorerController implements Controller {
	...
	public SqsExplorerController(SqsExplorerService sqsExplorerService, NavigationManager navigationManager) {
	...
	public void init() {
		this.carregarFilas();
		this.selecionarFilaDaRequisicao();
	}

	private void selecionarFilaDaRequisicao() {
		this.navigationManager.getRequestedResource(ApplicationRoute.SQS)
				.ifPresent(this::selecionarFilaPorNome);
	}
	...
	private void selecionarFilaPorNome(String nomeFila) {
		this.filas.stream()
				.filter(fila -> nomeFila.equals(fila.getNome()))
				.findFirst()
				.ifPresent(this::selecionarFila);
	}
